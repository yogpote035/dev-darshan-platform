...
import { User, Phone, Lock, AlertCircle, Loader } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    const result = await register(fullName, phone, password, confirmPassword, referralCode);
    setSubmitting(false);
    ...
    <div className="flex flex-col items-center justify-center min-h-[85vh] px-4 py-8">
      <div className="w-full max-w-md glass-card rounded-3xl p-6 md:p-8 shadow-2xl border border-white/5">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-400 text-xs mt-1.5">Join us to experience divine daily Darshans</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="fullName" className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">Full Name</label>
          ...
          <div>
            <label htmlFor="password" className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">Password (min 8 chars)</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-500">
                <Lock size={16} />
              </span>
          ...
          <div>
            <label htmlFor="confirmPassword" className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">Confirm Password</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-500">
                <Lock size={16} />
              </span>
          ...
          <div>
            <label htmlFor="referralCode" className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">Referral Code or Mobile Number (Optional)</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-500">
                <User size={16} />
              </span>
              <input
                ...
                placeholder="Enter referral code or referrer's mobile number"
                className="w-full pl-10 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl text-gray-200 text-sm focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
              />
            </div>
          </div>
